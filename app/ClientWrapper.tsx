...
export default function ClientWrapper({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth >= 1024) {
        ...
        setIsPanelOpen(true);  // Default open on large screens
      } else {
        setIsLargeScreen(false);
        setIsPanelOpen(false); // Default closed on smaller screens
      }
    };
  ...
  return (
    <div className="flex">
      {/* Left Panel */}
      <aside
        className={`fixed top-0 left-0 w-64 md:w-72 h-full bg-gray-100 dark:bg-gray-800 shadow-lg transform ${
          isPanelOpen ? 'translate-x-0' : '-translate-x-full'
        } transition-transform duration-300 z-20 md:z-30`}
      >
      ...
      {/* Main Content */}
      <div className={`flex-1 ${isPanelOpen ? 'ml-64 md:ml-72' : 'ml-0'} transition-all duration-300`}>
        {/* Navbar */}
        <nav className="bg-gradient-to-r from-blue-500 to-indigo-600 dark:from-gray-700 dark:to-gray-900 p-2 shadow-md fixed top-0 left-0 w-full z-30">
          <div className="container mx-auto flex flex-col md:flex-row items-center justify-between">
            ...
            <form className="flex items-center max-w-lg mx-auto flex-grow mt-2 md:mt-0">
              <input
                type="search"
                id="search-dropdown"
